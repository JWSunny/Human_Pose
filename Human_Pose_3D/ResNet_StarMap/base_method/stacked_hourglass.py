...
''' 主要通过 pytorch 实现， 最终使用的是 2级hourglass网络，每级网络使用的 4阶hourglass '''
...
class HourglassNet3D(nn.Module):
    ##### HourglassNet3D(2, 2, 256, 2)    4阶hourglass网络
    def __init__(self, nStack, nModules, nFeats, nRegModules):
        ...
        self.r5 = Residual(128, self.nFeats)  #### Residual(128, 256)
    ...
    def forward(self, x):
        ...
        for i in range(4):
            ...
            x = self.maxpool(x)
            print(x.shape)  ### [1, 256, 32, 32], [1, 256, 16, 16], [1, 256, 8, 8], [1, 256, 4, 4]

        print(x.shape)  ### [1, 256, 4, 4]
        x = x.view(x.size(0), -1)
        print(x.shape)  ### [1, 4096]    256*4*4
        reg = self.reg(x)
        out.append(reg)
